...
AI時代におけるプロンプト管理。ローカルで安全、簡単にプロンプトを記録・検索・管理できます。

## 主な特徴とメリット

* 簡単保存: `Ctrl+Shift+S` で今見ているプロンプトを即座に保存。
...
## Chrome拡張機能の利用
リポジトリ内の ChromeExt フォルダに含まれる拡張機能を利用すると、
どのWebページからでもプロンプトを素早く保存し、PromptVaultに送信できます。
...
プロンプトのタグ付け機能や編集機能の追加。

セキュリティ面での強化（ローカル/クラウドなどを想定）

モデル名だけでなく、プロジェクト名も保存できるようにする。
